...
class Explorer():
	...
	def get_driving_info(self, L):
        ...
        force_x = propelling_charge-force_x
                                          
        speed = self.pSpeed * math.sqrt(force_x ** 2 + force_y ** 2) * np.sign(force_x)
        angle = self.pAngle * math.atan2(-force_y, force_x) * np.sign(force_x)
                                                 
  		return (speed, angle)
                          
	def laser_callback(self, scan):
		cmd = get_driving_info(scan.ranges[180:901]) #returns a tuple with the speed and angle 
